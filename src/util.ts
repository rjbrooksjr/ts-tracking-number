...
const additionalCheck =
  (match: Partial<SerialData>) =>
  (a: Additional): boolean =>
    a.regex_group_name === 'ServiceType'
      ? a.lookup.some((x: Lookup) =>
          (x as LookupServiceType).matches_regex
            ? new RegExp((x as LookupServiceType).matches_regex).test(
                match.groups![a.regex_group_name],
              )
      ...
      : a.regex_group_name === 'CountryCode' ||
        a.regex_group_name === 'ShippingContainerType'
      ? a.lookup.some(
          (x) =>
            (x as MatchCourier).matches === match.groups![a.regex_group_name],
        )
      : true;

const matchTrackingData = (
  trackingNumber: string,
  regex: string | readonly string[],
): Partial<SerialData> => {
  const r = is(String, regex)
    ? (regex as string)
    : (regex as readonly string[]).join('');

  const match = new RegExp(`\\b${r}\\b`).exec(
    trackingNumber.replace(/[^a-zA-Z\d]/g, ''),
  );
...
const additional = (t: string, tracking: TrackingData): boolean =>
  tracking.additional
    ? tracking.additional.every(
        additionalCheck(matchTrackingData(t, tracking.regex)),
      )
    : true;

const dummy = (_serialData: SerialData): boolean => true;

const formatList = (tracking: string): readonly number[] =>
  pipe(
    split(''),
    map((x: string | number) =>
      isNaN(x as number)
...
const getSum = (
  parityFn: (v: number, k: number) => boolean,
...
const mod7 = ({ serial, checkDigit }: SerialData): boolean =>
  parseInt(serial) % 7 === parseInt(checkDigit);

const addWeight = (weightings: readonly number[], serial: string): number =>
  sum(
    zip(
...
const validateS10 = ({ serial, checkDigit }: SerialData): boolean => {
  ...
  const check = remainder === 1 ? 0 : remainder === 0 ? 5 : 11 - remainder;

  return check === parseInt(checkDigit);
};

const sumProductWithWeightingsAndModulo = ({
  serial,
  checkDigit,
  checksum,
}: SerialData): boolean =>
  (addWeight(checksum.weightings!, serial) % checksum.modulo1!) %
    checksum.modulo2! ===
  parseInt(checkDigit);

const validator = ({
  validation: { checksum },
}: TrackingData): ((x: SerialData) => boolean) =>
  checksum?.name === 'mod10'
    ? mod10
...
const formatSerial = (
  serial: string,
  numberFormat: SerialNumberFormat,
): string =>
  numberFormat.prepend_if &&
  new RegExp(numberFormat.prepend_if.matches_regex).test(serial)
...
const getTrackingList =
  (searchText: string) =>
  (trackingData: TrackingData): readonly string[] =>
    pipe<
      TrackingData,
    ...
    >(
      prop('regex'),
      ifElse(is(String), identity, join('')),
...
const getCourierList = (
  searchText: string,
  couriers: readonly TrackingCourier[],
): readonly string[] =>
  couriers.map(
...
const findTrackingMatches = (
  ...
  >(
    ...
    (a: readonly string[]) =>
      filter(
        (t: string) => none(test(new RegExp(`([a-zA-Z0-9 ]+)${t}$`)), a),
        // @ts-ignore Bad Dictionary Type
      )(a) as readonly string[],
    (a: readonly string[]) =>
      filter(
        (t: string) => none(test(new RegExp(`^${t}([a-zA-Z0-9 ]+)`)), a),
        // @ts-ignore Bad Dictionary Type
      )(a) as readonly string[],
  )(getCourierList(searchText, couriers));

// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
const getTrackingInternal = (trackingNumber: string) =>
  reduce(
    (prev: unknown, courier: TrackingCourier) =>
      prev ||
      reduce(
        (_: TrackingNumber | undefined, tn: TrackingData) => {
          const serialData = getSerialData(trackingNumber, tn);

          return serialData &&
            validator(tn)(serialData) &&
            additional(trackingNumber, tn)
...
): TrackingNumber | undefined => getTrackingInternal(trackingNumber)(couriers);

export const findTracking = (
  searchText: string,
  couriers?: readonly TrackingCourier[],
